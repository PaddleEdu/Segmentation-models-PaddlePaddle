...
def make_bilinear_weights(size, num_channels):
    ...
    w = paddle.zeros([num_channels, 1, size, size])
...
def conv_1x1(inp, oup):
    ...
    return nn.Conv2D(in_channels=inp, out_channels=oup, kernel_size=1, stride=1, padding=0, bias_attr=False)

def conv_1x1_bn(inp, oup):
    return nn.Sequential(
        nn.Conv2D(in_channels=inp, out_channels=oup, kernel_size=1, stride=1, padding=0, bias_attr=False),
...
def conv_bn(inp, oup, kernel, stride):
    return nn.Sequential(
        nn.Conv2D(in_channels=inp, out_channels=oup, kernel_size=kernel, stride=stride, padding=(kernel-1)//2, bias_attr=False),
        nn.BatchNorm2D(num_features=oup, epsilon=1e-05, momentum=0.1),
        nn.ReLU()
    )
...
def conv_dw(inp, oup, kernel, stride):
    return nn.Sequential(
        nn.Conv2D(inp, inp, kernel, stride, (kernel-1)//2, groups=inp, bias_attr=False),
...
class InvertedResidual(nn.Layer):
    def __init__(self, inp, oup, stride, expand_ratio, dilation=1):
        ...
        self.conv = nn.Sequential(
            # pw
            nn.Conv2D(inp, inp * expand_ratio, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias_attr=False),
            nn.BatchNorm2D(num_features=inp * expand_ratio, epsilon=1e-05, momentum=0.1),
            nn.ReLU(),
            # dw
            nn.Conv2D(inp * expand_ratio, inp * expand_ratio, 
                      kernel_size=3, stride=stride, padding=dilation, dilation=dilation,
                      groups=inp * expand_ratio, bias_attr=False),
            nn.BatchNorm2D(num_features=inp * expand_ratio, epsilon=1e-05, momentum=0.1),
            nn.ReLU(),
            # pw-linear
            nn.Conv2D(inp * expand_ratio, oup, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias_attr=False),
            nn.BatchNorm2D(num_features=oup, epsilon=1e-05, momentum=0.1),
        )
...
class ResidualBlock(nn.Layer):
    def __init__(self, inp, oup, stride=1):
        ...
        self.block = nn.Sequential(
            ...
            nn.Conv2D(in_channels=oup, out_channels=oup, kernel_size=3, stride=1, padding=1, groups=oup, bias_attr=False),
            nn.BatchNorm2D(num_features=oup, epsilon=1e-05, momentum=0.1),
            nn.ReLU(),
            nn.Conv2D(in_channels=oup, out_channels=oup, kernel_size=1, stride=1, padding=0, bias_attr=False),
            nn.BatchNorm2D(num_features=oup, epsilon=1e-05, momentum=0.1),
        )
        if inp == oup:
            self.residual = None
    ...
    def __init__(self, n_class=2, useUpsample=False, useDeconvGroup=False, addEdge=False, 
        ...
        if useUpsample == True:
            ...
            if useDeconvGroup == True:
                self.deconv1 = nn.Conv2DTranspose(self.depth(96), self.depth(96), groups=self.depth(96), 
                                                  kernel_size=4, stride=2, padding=1, bias_attr=False)
                self.deconv2 = nn.Conv2DTranspose(self.depth(32), self.depth(32), groups=self.depth(32), 
                                                  kernel_size=4, stride=2, padding=1, bias_attr=False)
                self.deconv3 = nn.Conv2DTranspose(self.depth(24), self.depth(24), groups=self.depth(24), 
                                                  kernel_size=4, stride=2, padding=1, bias_attr=False)
                self.deconv4 = nn.Conv2DTranspose(self.depth(16), self.depth(16), groups=self.depth(16), 
                                                  kernel_size=4, stride=2, padding=1, bias_attr=False)
                self.deconv5 = nn.Conv2DTranspose(self.depth(8),  self.depth(8),  groups=self.depth(8),  
                                                  kernel_size=4, stride=2, padding=1, bias_attr=False)
            else:
                self.deconv1 = nn.Conv2DTranspose(self.depth(96), self.depth(96), 
                                                  groups=1, kernel_size=4, stride=2, padding=1, bias_attr=False)
                self.deconv2 = nn.Conv2DTranspose(self.depth(32), self.depth(32), 
                                                  groups=1, kernel_size=4, stride=2, padding=1, bias_attr=False)
                self.deconv3 = nn.Conv2DTranspose(self.depth(24), self.depth(24), 
                                                  groups=1, kernel_size=4, stride=2, padding=1, bias_attr=False)
                self.deconv4 = nn.Conv2DTranspose(self.depth(16), self.depth(16), 
                                                  groups=1, kernel_size=4, stride=2, padding=1, bias_attr=False)
                self.deconv5 = nn.Conv2DTranspose(self.depth(8),  self.depth(8),  
                                                  groups=1, kernel_size=4, stride=2, padding=1, bias_attr=False)
        ...
        self.pred = nn.Conv2D(self.depth(8), n_class, 3, 1, 1, bias_attr=False)
        if self.addEdge == True:
            self.edge = nn.Conv2D(self.depth(8), n_class, 3, 1, 1, bias_attr=False)
    ...
    def _initialize_weights(self):
        for m in self.sublayers():
            if isinstance(m, nn.Conv2D):
                ...
                assert m._kernel_size[0] == m._kernel_size[1]
                initial_weight = make_bilinear_weights(m._kernel_size[0], m._out_channels) # same as caffe
                if self.useDeconvGroup == False:
                    initial_weight = make_bilinear_weights(m._kernel_size[0], m._out_channels).matmul(paddle.ones([m._in_channels,m._out_channels,m._kernel_size[0],m._kernel_size[0]]))
                m.weight = paddle.create_parameter(shape=m.weight.shape, dtype='float32', default_initializer=paddle.nn.initializer.Assign(initial_weight))
                if self.useDeconvGroup == True:
                    m.stop_gradient = True # use deconvolution as bilinear upsample
                    print ("freeze deconv")
        pass
