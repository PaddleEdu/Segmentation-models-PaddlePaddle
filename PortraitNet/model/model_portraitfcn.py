...
https://github.com/MattKleinsmith/portraitseg/blob/master/portraitseg/portraitfcn.py
'''
import numpy as np
import paddle
...
def get_upsampling_weight(in_channels, out_channels, kernel_size):
    ...
    return paddle.to_tensor(weight).float()
    ...
    def _initialize_weights(self):
        for m in self.sublayers():
            if isinstance(m, nn.Conv2d):
                # m.weight.data.zero_()
                m.weight = paddle.create_parameter(shape=m.weight.shape, dtype='float32', default_initializer=nn.initializer.Constant(value=0.0))
            ...
            if isinstance(m, nn.ConvTranspose2d):
                ...
                initial_weight = get_upsampling_weight(
                    m._in_channels, m._out_channels, m._kernel_size[0])
...
class PortraitFCNPlus(PortraitFCN):
    def __init__(self, load_weights=False, dropout=0.5):
        ...
        if load_weights:
            ...
            self.load_state_dict(paddle.load(path_to_weights))
        ...
        size = (c_in, k, k)
        n = k * k * c_out
        superkernels = paddle.to_tensor(np.zeros((c_out, c_in, k, k)))
        for i, kernel in enumerate(kernels):
            superkernel = paddle.to_tensor(np.random.normal(0, np.sqrt(2./n),
                                                            size=size))
